// Modified from upstream KyleMayes/install-llvm-action: ported to ESM (type-only
// imports, `import.meta.url` in place of `__dirname`) and the lodash chain in
// `extractVersionNumber` replaced with a plain loop. See ../NOTICE.
...
function extractVersionNumber(release: Release): Version | undefined {
  for (const pattern of VERSION_PATTERNS) {
    const version = pattern.exec(release.tag_name)?.groups?.version;
    if (version) {
      return version;
    }
  }
...
async function generate() {
  ...
  writeFileSync(resolve(dirname(fileURLToPath(import.meta.url)), "assets.json"), json);
}
