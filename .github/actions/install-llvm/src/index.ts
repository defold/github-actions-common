// Modified from upstream KyleMayes/install-llvm-action: ported to ESM and the
// asset manifest is now imported (and so inlined into the bundle by ncc) rather
// than read from disk at runtime, because `__dirname` does not exist in ESM.
// See ../NOTICE.
...
const ASSETS: Record<string, Record<string, Record<string, string>>> = assets;
 ...
 */
export function getSpecificVersions(specificVersions: string[], version: string): string[] {
